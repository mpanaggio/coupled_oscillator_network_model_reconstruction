...
def learn_model(params,trainX1,trainX2,trainY,testX1,testX2,testY):
    ...
    ## compute predicitions: k1 term for RK2 method
    dt=params['dt']
    ypred_k1=dt*v

    ## compute phase velocity using k1 (RK2)
    v1,fout=get_vel(A,omega,K,add_dim(get_diff_mat(X2+0.5*ypred_k1)),params)

    ## compute predicitions: RK2 method using k2 term
    ypred=predict_phases(X2,v1,params)
