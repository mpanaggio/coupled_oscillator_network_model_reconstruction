...
#dynamictype="ring_chimera"
dynamictype="twisted"

if dynamictype=="ring_chimera":
    pertsize=0.2
    num_osc=40#40
    ...
    downsample=1
    warmuptime=0
    noise_level=0.001 # post solution noise added
    ...
    P=int(num_osc*frac)
    K=num_osc/(2*P)
    A=chim.generate_ring(num_osc,P)
    ...
    #IC_original=np.array([0,phi2,0,np.pi,phi2,np.pi])-1
    #IC_original=np.concatenate([IC_original,IC_original])
    #
    #IC_original=(np.random.rand(num_osc)*2-1)*np.pi#
    winding=0
    IC_original=chim.generate_ic_ring_chimera(
            num_osc,
            invwidth=30,
            exponent=2,
            seed=random_seed)+2*np.pi*winding*np.arange(0,1,1/num_osc)
    IC_original=np.angle(np.exp(1j*IC_original))
elif dynamictype=="twisted":
    pertsize=0.00
    num_osc=20#40
    mu_freq=1.0  # mean natural frequency
    sigma_freq=0.001 # std natural frequency
    ...
    ## define numerical solution parameters
    dt=0.1    # time step for numerical solution
    tmax=2000*dt    # maximum time for numerical solution
    downsample=10
    warmuptime=0
    ...
    #IC_original=(np.random.rand(num_osc)*2-1)*np.pi#
    winding=1
    IC_original=2*np.pi*winding*np.arange(0,1,1/num_osc)
...
plt.figure()
plt.plot((phases[-1,:]-phases[ind_post_trans,:])/(t_all[-1]-t_all[ind_post_trans]))
plt.xlabel('index')
...
for rep in range(num_repeats):
    ...
    R1,Psi1=lk.get_op(cur_phases)
    
    ax1=plt.subplot(2,1,1)
    plt.plot(cur_t,cur_phases,c='b')
